...
export class AdjustTimeLogStopDate1644491785525 implements MigrationInterface {
	...
	public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ...
        `);
		const timeLogs = chain(timelogs)
			.groupBy((log: any) => log.time_log_id)
			.value();
		for await (const [timeLogId, timeSlots] of Object.entries(timeLogs) as [
			string,
			Array<{ time_slot_id: string }>
		][]) {
			const timeSlotsIds = timeSlots.map((timeSlot: any) => timeSlot.time_slot_id).filter(Boolean);
			...
			if (isEmpty(timeSlotsIds) && logDifference > 10) {
				await queryRunner.connection.manager.query(
					`UPDATE "time_log" SET
                        "stoppedAt" = ?
                    WHERE
