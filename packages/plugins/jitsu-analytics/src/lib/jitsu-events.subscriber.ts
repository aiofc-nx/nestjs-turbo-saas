...
export class JitsuEventsSubscriber extends BaseEntityEventSubscriber {
	...
	async identify(id: string | object, traits?: Record<string, any> | null): Promise<any> {
		...
		const userId = typeof id === 'string' ? id : JSON.stringify(id);
		// Convert traits to the expected type (string | Traits)
		const traitsParam = traits as any;

		return await this.jitsuAnalytics.identify(userId, traitsParam);
	}
